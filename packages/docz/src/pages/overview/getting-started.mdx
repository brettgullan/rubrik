...
`@rubrik/core` provides a base set of  components including Buttons/Links, a Type components (based on the BBC GEL typescale), Image and Picture, and useful structural/layout components including Card, Tile and Grid components.

Additional `@rubrik` packages support more specific use-cases and depend upon `rubrik/core`.
...
### npm
```bash
$ npm install @rubrik/core @rubrik/media @rubrik/forms
```

### yarn
```bash
$ yarn add @rubrik/core @rubrik/media @rubrik/forms
```
...
```javascript
import { Button, Grid, Type } from '@rubrik/core'
```
...
However, all Rubrik components depend upon a `theme`.
